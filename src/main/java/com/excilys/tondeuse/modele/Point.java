...
public class Point {
    ...
    public Point(int x, int y) throws ModelException {
        if ( x < 0) {
            throw new ModelException("La position selon l'axe Ouest-Est (x) ne peut pas être négative !");
        }
        if ( y < 0) {
            throw new ModelException("La position selon l'axe Nord-Sud (y) ne peut pas être négative !");
        }
        this.x = x;
        this.y = y;
    }
